...
PATTERNS['vesica-piscis'] = function(opts = {}) {
  ...
  const R = size / 2 - 2;
  const offset = R / 2;
  return wrap([circ(cx - offset, cy, R), circ(cx + offset, cy, R)], opts);
};
...
PATTERNS['golden-spiral'] = function(opts = {}) {
  ...
  return wrap([circ(cx, cy, maxR), pth(`M${pts.join(' L')}`, 'fill="none"')], opts);
};
...
PATTERNS['sri-yantra'] = function(opts = {}) {
  ...
  for (const s of upTriangles) {
    const h = R * s, base = h * 1.15;
    els.push(pth(`M${r2(cx)},${r2(cy - h * 0.7)} L${r2(cx + base / 2)},${r2(cy + h * 0.5)} L${r2(cx - base / 2)},${r2(cy + h * 0.5)} Z`));
  }
...
PATTERNS['compass-rose'] = function(opts = {}) {
  ...
  for (let i = 0; i < points; i++) {
    ...
    const [rv] = [polar(cx, cy, R * 0.15, angle + 360 / points / 2)];
    els.push(pth(`M${tip[0]},${tip[1]} L${l[0]},${l[1]} L${rv[0]},${rv[1]} Z`, 'fill="#fff" stroke-width="0.5"'));
  }
...
PATTERNS['tree-of-life'] = function(opts = {}) {
  ...
  const R = 3.5;
  const nodes = [
    [cx, 4], [cx - 8, 12], [cx + 8, 12], [cx, 16],
    [cx - 8, 24], [cx + 8, 24], [cx, 28],
    [cx - 8, 36], [cx + 8, 36], [cx, 44],
  ];
  const edges = [
    [0, 1], [0, 2], [1, 2], [1, 3], [2, 3],
    [1, 4], [2, 5], [3, 4], [3, 5], [4, 5],
...
const AVAILABLE_PATTERNS = Object.keys(PATTERNS).sort();
...
const SVG_SYSTEM_PROMPT = `You are an SVG icon designer. Generate a SINGLE <svg> element with these constraints:
- viewBox="0 0 48 48" (48x48 coordinate space)
...
async function generateWithLLM(description, model) {
...
const CDN_BASE = 'https://cdn.jsdelivr.net/gh/wilsr7000/podscan-node@main/knowledge-twin/icons';

function iconUrl(pattern) {
  if (!pattern) return null;
  return `${CDN_BASE}/${pattern}.svg`;
}
...
class CreateIcon extends Step {
  async runStep() {
    const mode = this.data.mode || 'pattern';
    ...
    try {
      ...
      if (mode === 'pattern') {
        ...
        if (!pattern) {
          return this.exitStep('error', {
            ...
            message: 'Pattern name is required in pattern mode.',
        ...
        const opts = this.data.patternOptions || {};
        svg = generator(typeof opts === 'string' ? JSON.parse(opts) : opts);
        resultMeta = { mode: 'pattern', pattern, options: opts };
        ...
        if (!prompt) {
          return this.exitStep('error', {
            code: 'MISSING_PROMPT',
            message: 'Prompt is required in llm mode.',
          });
        }

        svg = await generateWithLLM(prompt, this.data.llmModel);
        resultMeta = { mode: 'llm', prompt };

      } else {
        return this.exitStep('error', {
          code: 'INVALID_MODE',
          message: `Invalid mode "${mode}". Use "pattern" or "llm".`,
        });
      }

      return this.exitStep('next', {
        svg,
        iconUrl: mode === 'pattern' ? iconUrl(resultMeta.pattern) : null,
        ...resultMeta,
        charCount: svg.length,
        availablePatterns: AVAILABLE_PATTERNS,
      });
      ...
      if (this.data.processError) {
        return this.exitStep('error', { code: 'STEP_ERROR', message: err.message });
      }
      throw err;
    }
  }
